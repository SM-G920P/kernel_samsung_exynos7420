...
# kernel build script modified by mtb3000gt
...
ZIP_VER=`sed -n '8p' arter`
DEVICE_VER=`sed -n '4p' arter`
VER=`sed -n '6p' arter`
SU_VER=`sed -n '14p' arter`
KERNEL_NAME="$ZIP_VER$DEVICE_VER$VER"
TOOLCHAIN=`sed -n '12p' arter`
BUILD_CROSS_COMPILE=$TOOLCHAIN_DIR/$TOOLCHAIN


#vars
KERNEL_DEFCONFIG=arter_defconfig
BASE_VER=`sed -n '2p' arter`
DEVICE_VER=`sed -n '4p' arter`
VER=`sed -n '6p' arter`
...
BUILD_KERNEL()
{
	...
	export KBUILD_BUILD_USER=arter
